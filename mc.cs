...
namespace CalculatorNotepad
{
    ...
    public class mc
    {
        ...
        public static void Init()
        {
            ...
            // bitwise operators ( reverse logic from C# .. use double sign for bitwise .. so that ^^ is allowed for xor )
            ...
            addFunc("^^", new mcFuncParse("^^", mcFuncParamType.Between, 2, 10, new mcFunc(args => nmc.bitxor(args)), "Returns bitwise XOR from the scecified integer list\r\n   if vectors specified, find XOR of vector values", "bitxor ( a[,b[,c,...]] )"));
            // logic operators
            addFunc("|", new mcFuncParse("|", mcFuncParamType.Between, 2, 10, new mcFunc(args => new mcValue(args[0].isTrue() || args[1].isTrue() ? 1 : 0))));
            addFunc("&", new mcFuncParse("&", mcFuncParamType.Between, 2, 10, new mcFunc(args => args[0].isTrue() && args[1].isTrue() ? 1 : 0)));
            ...
            addFunc("if", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => args[0].isTrue() ? args[1] : args[2]), "Check condition, then evaluate expression. Has three versions:\r\nif(cond) expIFtrue [else expIFfalse]; \r\n   return expIFtrue if cond==true, otherwise returns expIFfalse\r\n   in case there is no 'else' section, returns zero\r\nif( cond, expIFtrue [, expIFfalse] )\r\n   same logic, different syntax", "if (cond) expTrue [else expFalse];  <or>  if( cond ,resIfTrue [,resIfFalse] )"));
            ...
            addFunc("find_min", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => nmc.find_min(args)), "returns X from [left,right] range for which f(X) has minimal value\r\n    - assume single unimodal minimum of function f(x) within [left,right] range\r\n    - find_min((x)=> (x-3)^2, -3, 7) -> 3  ", "find_min(  f(x) , left , right [, tolerance=1e-12 ] ) "));
            addFunc("find_max", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => nmc.find_max(args)), "returns X from [left,right] range for which f(X) has maximal value\r\n    - assume single unimodal maximum of function f(x) within [left,right] range\r\n    - find_max((x)=> -(x-1)^2, -3, 3) -> 1  ", "find_max(  f(x) , left , right [, tolerance=1e-12 ] ) "));
