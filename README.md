...
- function return uses standard `return` keyword, and suport mid-function returns
...
it is interpreted as opposed to compiled. That has impact on its performanse, but allows language to remain as flexible as needed. 
...
- `vLen(vector)` return number of elements in vector ( thus counting how many distinct ressults we had in one trial)
...
In this case that would require solving this as mathematical problem and comming up with formula for pMath(M,T,N). 
