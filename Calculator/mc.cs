...
namespace CalculatorNotepad
{
    ...
    public class mc
    {
        ...
        public static void Init()
        {
            ...
            addFunc("vReverse", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => nmc.vReverse(args)), "reverse order of elements in vector\r\n    vReverse(vec(7,9,3,5)) => vec(5,3,9,7)", "vReverse(vec)"));
            // extrapolation functions
            addFunc("extrapolate", new mcFuncParse("", mcFuncParamType.Func, 3, 90, new mcFunc(args => nmc.extrapolate(args)), "Find Y value given X value and two vectors: vX-values and vY-values\r\n    Linear extrapolation based on (vX, vY) points\r\n    See also: 'areapolate' and 'avgpolate'\r\n    extrapolate( 3, vec(1,5), vec( 100,500)) =>  300", "extrapolate(X, vecX, vecY)"));
            addFunc("areapolate", new mcFuncParse("", mcFuncParamType.Func, 4, 90, new mcFunc(args => nmc.areapolate(args)), "Calculate area of extrapolated function between X1 and X2\r\n    Linear extrapolation for Y=f(X) based on (vX, vY) points\r\n    See also: 'avgpolate' and 'extrapolate'\r\n    areapolate( 2, 4, vec(1,5), vec( 100,500)) =>  600", "areapolate(X1,X2, vecX,vecY)"));
            addFunc("avgpolate", new mcFuncParse("", mcFuncParamType.Func, 4, 90, new mcFunc(args => nmc.avgpolate(args)), "Average Y value between X1 and X2, weighted\r\n    Same as `areapolate(X1,X2,..)/(X2-X1)`\r\n    Linear extrapolation for Y=f(X) based on (vX, vY), weighted by segment lengths\r\n    See also: 'areapolate' and 'extrapolate'\r\n    avgpolate( 2, 4, vec(1,5), vec( 100,500)) =>  300", "avgpolate(X1,X2, vecX,vecY)"));
            ...
            addFunc("LambertW", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => Number.LambertW(args[0].Number)), "LambertW or ProductLog, returns w such that w*e^w=x\r\n  Same as ProductLog(x)", " LambertW ( x )"));
            addFunc("ProductLog", new mcFuncParse("", mcFuncParamType.Func, 1, 90, new mcFunc(args => Number.LambertW(args[0].Number)), "ProductLog or LambertW, returns w such that w*e^w=x\r\n  Same as LambertW(x)", " ProductLog ( x )"));
